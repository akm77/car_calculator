...
- 🔄 **Hot Reload Commands**: Реализованы команды для перезагрузки конфигов без перезапуска сервера
  - `/reload_configs` — очистка кэша и перезагрузка всех конфигов в runtime
  - `/config_status` — проверка текущей версии конфигов (hash + timestamp)
  - `/config_diff` — сравнение конфигов в памяти и на диске
  - Функция `reload_configs()` в `app/core/settings.py` с полными метриками

- 🏷️ **Config Versioning**: Система версионирования конфигурационных файлов
  - SHA-256 hash для каждой загрузки конфигов (16-символьный truncated hash)
  - Timestamp в ISO 8601 format (UTC) для отслеживания времени загрузки
  - Hash comparison для определения изменений между памятью и диском
  - Метрики: `config_count`, `old_hash`, `new_hash`, `loaded_at`, `load_time_ms`, `hash_changed`

- ⚡ **Performance Metrics**: Детальная информация о производительности reload
  - Измерение времени загрузки конфигов (`load_time_ms`)
  - Отслеживание изменений hash (`hash_changed` boolean flag)
  - Логирование всех операций с structured logging (structlog)
  - Graceful degradation при ошибках валидации

- 🛡️ **Graceful Error Handling**: Защита от ошибок при hot reload
  - Сохранение старых конфигов перед попыткой загрузки новых
  - При ошибке загрузки старые конфиги остаются в памяти
  - Информативные сообщения об ошибках с типом exception и деталями
  - Использование `logger.exception()` для полного traceback

- 🧪 **Unit Tests**: 5 тестов для core функциональности `reload_configs()`
  - `test_reload_configs_success` — успешная перезагрузка
  - `test_reload_configs_with_hash_change` — reload с изменением hash
  - `test_reload_configs_failure` — обработка ошибок валидации
  - `test_reload_configs_no_old_configs` — reload без старых конфигов в кэше
  - `test_reload_metrics_structure` — валидация структуры метрик

- 📚 **Documentation Updates**: Обновлена документация по hot reload
  - Добавлен workflow для hot reload в `docs/CONFIG_MANAGEMENT.md`
  - Примеры использования команд `/reload_configs`, `/config_status`, `/config_diff`
  - Объяснение zero-downtime deployment стратегии
...
- **Завершён полный цикл тестирования (SPRINT TEST-01 → TEST-07)**:
  - Создано **619 тестов** (было ~50): 477 юнит-тестов + 142 функциональных теста
  - Достигнут **91% coverage** (было ~60%): models 100%, tariff_tables 100%, routes 99%, engine 94%
  - Все HIGH priority тесты созданы и проходят (0 failed, 618 passed, 1 skipped)
  
- **Финальная статистика по покрытию**:
  - **Models validation** (148 тестов): CalculationRequest, CalculationMeta, CostBreakdown, CalculationResult
  - **Rounding & utilities** (186 тестов): round_rub, to_decimal, quantize4, age categories, duty rates
  - **Engine logic** (143 теста): helpers (_compute_duty, _commission, _freight, etc.) + integration (calculate)
  - **API functional** (142 теста): REST endpoints, validation, bank commission, CBR service
  
- **100% покрытие SPECIFICATION.md**:
  - Раздел 2 (входные данные): валидация всех полей с граничными случаями
  - Раздел 3 (классификация авто): age_category (lt3, 3_5, gt5) с boundaries
  - Раздел 4.1 (страновые расходы): все 5 стран (japan тиры JPY, остальные фикс USD)
  - Раздел 4.2 (фрахт): japan санкционный/несанкционный, uae open/container
  - Раздел 4.3.2 (утильсбор v2): 2D таблица, HP→kW конвертация, все диапазоны
  - Раздел 4.3.3 (пошлины): LT3 value brackets, 3_5/GT5 CC bands, все границы
  - Раздел 4.3.4 (ЭРА-ГЛОНАСС): базовая ставка 45,000 руб (известный баг: не входит в total)
  - Раздел 4.4 (комиссия компании): default 1000 USD, UAE=0, конвертация
  - Раздел 4.5 (банковская комиссия): effective_rate, meta.detailed_rates_used, display format
  
- **Качество тестов**:
  - ✅ Изоляция: fixtures для конфигов, нет внешних зависимостей в unit тестах
  - ✅ Детерминированность: фиксированные даты, фиксированные курсы
  - ✅ Понятность: описательные имена, docstrings, параметризация с ids
  - ✅ Полнота: happy/sad/boundary/edge cases + инварианты (total=sum, UAE=0)
  
- **Создана документация**:
  - `docs/sprints/TEST_FINAL_REPORT.md` — полный отчёт (статистика, покрытие по SPEC, TODO, качество)
  - `docs/sprints/TEST_MAINTENANCE_CHECKLIST.md` — руководство по поддержке тестов
  - `README.md` — добавлена секция Testing (команды, структура, coverage)
  - `docs/rpg.yaml` — обновлена секция tests (финальная статистика)
  
- **cases.yml обновлён** (v2.0.0):
  - Было: 13 кейсов
  - Стало: **46 кейсов** (все страны, возрастные категории, граничные случаи, power ranges)
  
- **Известные проблемы задокументированы**:
  - ERA-GLONASS не включён в total_rub (баг зафиксирован, не блокирует релиз)
  - 2 MEDIUM priority gaps: static file serving, utilization v2 fallback
  - 2 LOW priority gaps deferred: middleware, bot handlers (ручные тесты достаточны)

### ✅ Testing (SPRINT TEST-02)

- **Создано 148 юнит-тестов для моделей данных** (2025-12-16):
  - `tests/unit/test_models_validation.py` (101 тест) — полное покрытие валидации `CalculationRequest`: все 9 полей (year, engine_cc, engine_power_hp, purchase_price, currency, country, freight_type, vehicle_type, sanctions_unknown), 2 валидатора (validate_year, normalize_currency), граничные значения и типы данных;
  - `tests/unit/test_models_meta.py` (30 тестов) — покрытие `CalculationMeta`, `RateUsage`, `WarningItem`: конвертация hp→kW (0.7355), структуры rates_used и detailed_rates_used, все опциональные поля (customs_value_eur, duty_percent, utilization_coefficient);
  - `tests/unit/test_models_breakdown.py` (17 тестов) — покрытие `CostBreakdown` (все 9 компонентов + инвариант целостности: total_rub = сумма компонентов) и `CalculationResult` (композиция request+meta+breakdown);
- **Coverage моделей:** ~95-100% для app/calculation/models.py;
- **Документация:** обновлены docs/rpg.yaml (добавлены 3 записи в tests.unit_tests), docs/sprints/SPRINT_TEST_02_UNIT_MODELS.md (отмечен статус завершения), создан docs/sprints/SPRINT_TEST_02_COMPLETION.md с полным отчётом;
- **Качество:** все тесты используют параметризацию, изолированы, проверяют граничные случаи, соответствуют PEP 8, без ошибок линтера.
...
**Исправление расчета таможенной пошлины для авто ≤3 лет:**
- **Критический баг:** Брэкеты пошлин lt3 в `config/duties.yml` были указаны в рублях/100 вместо EUR
- **Причина:** Ошибочная конвертация официальных данных ЕАЭС (пороги уже в EUR, не требуют деления на 100)
- **Исправление:** Обновлены все 6 брэкетов lt3 на официальные значения:
  - Было: 3,250 / 6,500 / 16,250 / 32,500 / 65,000 EUR (НЕВЕРНО)
  - Стало: 8,500 / 16,700 / 42,300 / 84,500 / 169,000 EUR (ПРАВИЛЬНО)
...
- **Критический баг:** При нажатии кнопки "Поделиться результатом" бот выдавал ошибку "Не указана мощность двигателя", хотя форма была полностью заполнена
- **Причина:** В функции `shareResult()` не передавалось поле `engine_power_hp` в объекте `telegramData`, отправляемом боту через WebApp
- **Исправление:** Добавлено поле `engine_power_hp: r.engine_power_hp` в два места:
  1. В основной payload `telegramData` (строка ~1207)
  2. В минимальный payload при превышении размера (строка ~333)
- **Файл:** `app/webapp/index.html`
- **Затронутая функция:** `shareResult()` и `telegram.sendDataSafe()`
