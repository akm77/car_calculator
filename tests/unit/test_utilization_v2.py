...
@pytest.mark.parametrize("age_category,engine_cc,engine_power_hp,expected_coefficient", [
    ...
    ("lt3", 2500, 180, 115.34),  # 180hp = 132.39kW → 117.69-139.75kW → коэфф. 115.34 (обновлено 2025)
    ("gt5", 2500, 200, 175.08),  # 200hp = 147.1kW → 139.76-161.82kW → коэфф. 175.08 (обновлено 2025)

    # Диапазон 3001-3500 cc
    ("lt3", 3200, 250, 137.16),  # 250hp = 183.88kW → 161.83-183.88kW → коэфф. 137.16 (обновлено 2025)
    ("gt5", 3200, 260, 207.24),  # 260hp = 191.23kW → 183.89-205.94kW → коэфф. 207.24 (обновлено 2025)

    # Диапазон >3500 cc
    ("gt5", 4000, 300, 236.64),  # 300hp = 220.65kW → 197.2-228.0kW → коэфф. 236.64 (обновлено 2025)
