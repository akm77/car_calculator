...
#### Уровень 3: Детальная информация (по клику на ℹ️)
```
┌────────────────────────────────────────────┐
...
## 💱 Информационный блок о курсе (bank_commission)

Помимо подсказок по возрасту и мощности, в карточке результата WebApp выводится
неподсвеченная строка с информацией о валютном курсе, использованном при расчёте
(с учётом банковской надбавки к курсу):

- Расположение: блок `meta-info` на карточке результата, сразу после строки со страной.
- Формат:
  - с комиссией: `Курс валюты: USD/RUB = 90 + 3%`;
  - без комиссии: `Курс валюты: USD/RUB = 90`.
- Источник: `meta.detailed_rates_used[*].display` из ответа `/api/calculate`.
- Цель: показать пользователю, что итоговая сумма уже учла возможную
  банковскую надбавку к курсу, без раскрытия самой комиссии в рублях.

Эта строка **не является “подсказкой”** в смысле цветовых статусов и не
подсвечивается цветом — это статичная информационная метка рядом с другими
метаданными (возраст, объём, утильсбор и т.п.).
