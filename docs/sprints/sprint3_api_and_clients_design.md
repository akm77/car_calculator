...
### 2.1. Минимальный контракт `meta.rates_used` для клиентов

В рамках Sprint 3 зафиксирован следующий принцип, который мы **сохраняем** и в последующих спринтах:

- Клиентам (WebApp и бот) **не нужно** знать внутреннюю структуру курсов и банковской комиссии;
- Для них `meta.rates_used` представляет собой источник **готовой текстовой строки курса**:
  - `"USD/RUB = 78.95"` — если банковская комиссия выключена или равна 0%;
  - `"USD/RUB = 78.95 + 1%"` — если комиссия включена и > 0.
- Формирование этой строки — **ответственность сервера**:
  - движок и/или API собирают `base_rate`, `bank_commission_percent`, `effective_rate` и форматируют строку;
  - клиенты не выполняют обратный расчёт и не выделяют «чистый» курс и сумму комиссии.

Подробная логика и внутренняя структура описаны в `docs/SPECIFICATION.md`, раздел 4.5.4
(модель `RateUsage` и поле `CalculationMeta.detailed_rates_used`).
