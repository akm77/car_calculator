...
## 🎯 Использование detailed_rates_used на клиентах (WebApp и бот)

И WebApp, и Telegram-бот читают поле `meta.detailed_rates_used` и используют заранее
подготовленную на бэкенде строку `RateUsage.display` для отображения курса:

- при `bank_commission_percent == 0`:
  - `"USD/RUB = 90.0"`
- при ненулевой комиссии:
  - `"USD/RUB = 90.0 + 2.0%"`

Правила выбора валюты для отображения:
- в первую очередь используется валюта покупки `request.currency`, если по ней
  есть запись в `detailed_rates_used`;
- если записи нет или покупка в RUB — клиенты пробуют последовательность
  `USD → EUR → JPY` и берут первую доступную;
- если ни одна не найдена — берётся первая доступная запись из `detailed_rates_used`.

Форматы на клиентах:
- **WebApp**:
  - в блоке `metaInfo` под заголовком страны выводится строка:
    - `Курс валюты: USD/RUB = 90` или
    - `Курс валюты: USD/RUB = 90 + 3%`;
  - в тексте «Поделиться результатом» добавляется строка:
    - `Курс для расчета: USD/RUB = 90 [+ 3%]`.
- **Telegram‑бот**:
  - в итоговом сообщении после строки с ценой добавляется строка:
    - `💱 Курс: USD/RUB = 90` или
    - `💱 Курс: USD/RUB = 90 + 3%`.

Клиенты **не рассчитывают комиссию самостоятельно** и не используют `effective_rate`
напрямую — они полагаются на строку `display` или, при её отсутствии, аккуратно
восстанавливают формат из `base_rate` и `bank_commission_percent`.
