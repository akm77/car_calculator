...
Применяется **по данным Таможенного союза ЕАЭС** (официальные ставки 2025).
...
> Этот подпункт описывает **спецификацию** будущего поведения. На момент версии v2.0 код ещё не использует эти настройки, они формализованы в рамках спринта 1 (bank_commission_config).
...
> - содержимое `CalculationMeta.rates_used` (см. ниже).
...
> Детальный алгоритм применения будет спроектирован в спринте 2 (engine & meta design). Здесь фиксируется только *контракт*.
...
- Банковская комиссия применяется к **операциям в иностранной валюте** до конвертации в RUB:
  ...
  - комиссия компании, если в будущем она будет выражена в валюте (сейчас — фиксированная 1000 USD без надбавк��).
...
Модель `CalculationMeta` (см. `app/calculation/models.py`) уже содержит поле `rates_used`, в котором трекаются используемые курсы валют.

Будущее требование к структуре `meta.rates_used`:

- Для каждой используемой валюты (например, `USD_RUB`, `JPY_RUB` и т.п.) должны быть доступны по крайней мере следующие логические поля:
