...
### 4.5.4.1. Логическая внутренняя структура `rates_used`

Во внутренней Python‑модели (`app/calculation/models.py`) поле `CalculationMeta.detailed_rates_used`
описывается типом `dict[str, RateUsage]`, где `RateUsage` содержит:

- `base_rate: float` — базовый курс валютной пары `XXX_RUB` без учёта комиссии банка (значение из `rates_conf["currencies"]`);
- `effective_rate: float` — эффективный курс с учётом надбавки банка:
  `effective_rate = base_rate * (1 + bank_commission_percent / 100)`;
- `bank_commission_percent: float` — фактически применённый процент банковской комиссии (0, если комиссия отключена);
- `display: str` — человекочитаемая строка курса для UI, формируемая на сервере в формате
  `"USD/RUB = BASE"` или `"USD/RUB = BASE + PERCENT%"`.

Поле `CalculationMeta.rates_used: dict[str, float]` сохраняется для обратной совместимости
и содержит **числовое** представление курсов `XXX_RUB` (в текущей реализации — эффективные
курсы, согласованные с расчётом `total_rub`). Новые клиенты, которым нужны детали комиссии,
должны ориентироваться на `detailed_rates_used` и поле `display`, а не на «сырое» число
из `rates_used`.

### 4.5.4.2. Минимальный JSON‑контракт для клиентов

В ответе `/api/calculate` для клиентов (WebApp и Telegram‑бота) гарантируется следующее:

- В объекте `meta` присутствует поле `rates_used` с ключами вида `USD_RUB`, `JPY_RUB` и т.п.;
- Значения под этими ключами — числа (float/int), пригодные для внутренней диагностики,
  но не являющиеся обязательной частью пользовательского UI‑контракта;
- Для целей UI сервер формирует человекочитаемую строку курса в `CalculationMeta.detailed_rates_used[CODE].display`;
- Клиентам достаточно использовать именно `display` (на стороне сервера она может быть
  продублирована в `meta.rates_used` отдельным ключом, но это не меняет минимальный контракт).

Минимальный контракт для UI‑клиентов:

- Должна существовать хотя бы одна человекочитаемая строка курса, пригодная для прямого вывода в UI;
- Формат строки: `"USD/RUB = BASE"` при `bank_commission_percent == 0` и
  `"USD/RUB = BASE + PERCENT%"` при `bank_commission_percent > 0`;
- Клиенты **не обязаны** знать, где именно она хранится внутри `meta` (в `rates_used` или
  в `detailed_rates_used.display`), но текущая реализация предоставляет её через `display`.

### 4.5.4.3. Обратная совместимость

- Формат JSON **не расширяется в ширину** для ключа `meta` в рамках этого спринта: новые внутренние поля
  (`detailed_rates_used`) не считаются частью публичного контракта и могут меняться без уведомления клиентов;
- Старые клиенты, которые никогда не обращаются к `meta.rates_used`, продолжают работать как раньше,
  используя только поля из `breakdown`;
- При `bank_commission_percent = 0` строка курса в `display` **не содержит** суффикс `+ X%`, чтобы не создавать иллюзию
  надбавки, которой нет;
- При ненулевой комиссии обязательной частью человекочитаемой строки является `+ PERCENT%`.
