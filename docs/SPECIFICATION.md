...
> Этот подпункт описывает **спецификацию** поведения. В актуальной реализации банковская комиссия уже применяется как надбавка к валютному курсу для всех релевантных валютных операций, включая конвертацию фиксированной комиссии компании 1000 USD в RUB.

#### 4.5.1. Определение

- **Банковская комиссия** — это *процентная надбавка к валютному курсу*, применяемая к операциям в иностранной валюте **до** конвертации в RUB.
- Базовая формула (логический контракт):
  - `effective_rate = base_rate × (1 + bank_commission_percent / 100)`
- Где:
  - `base_rate` — курс валюты к RUB из конфигурации/источника курсов (`rates.yml` +, опционально, CBR);
  - `bank_commission_percent` — глобальный процент банковской комиссии (из конфигурации комиссий);
  - `effective_rate` — итоговый курс, фактически использованный при расчёте сумм в RUB.

> Эффект банковской комиссии **не выводится** отдельной строкой в API и UI. Он отражается только через:
> - изменение `breakdown.total_rub` (итоговая сумма);
> - содержимое `CalculationMeta.rates_used` / `CalculationMeta.detailed_rates_used` (см. ниже).
...
#### 4.5.4. Связь с метаданными `CalculationMeta`

Модель `CalculationMeta` (см. `app/calculation/models.py`) содержит поля `rates_used` и `detailed_rates_used`, в которых трекаются используемые курсы валют.

Будущее (и текущей реализации соответствующее) требование к структуре:

- Для каждой используемой валюты (например, `USD`, `JPY` и т.п.) должны быть доступны по крайней мере следующие логические поля:
  - `base_rate` — исходный курс из конфигурации/источника;
  - `bank_commission_percent` — применённый процент банковской комиссии (0, если отключено);
  - `effective_rate` — итоговый курс после применения банковской комиссии;
  - `display` — строковое представление для UI (`"USD/RUB = BASE + P%"` или `"USD/RUB = BASE"` при нулевой комиссии).

Пример целевой структуры (JSON-представление):
...
> В этом спринте **структура JSON-ответа API не меняется**. Этот блок задаёт контракт для будущего развития модели и метаданных.
